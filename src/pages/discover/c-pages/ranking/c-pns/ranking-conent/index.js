...
import {getSongListDetailAction,getSongListTitle} from '../../store/actionCreators'
...
export default memo(function WYRaningConent() {
    ...
    // 获取数据
    const {songListDetail,err} = useSelector(state => ({
  ...
  return (
    <RankingConentWrapper err={err}>
      ...
      {/* 歌曲数据 */} 
      <WYSongListTitleCover  showPlay = {false}
                        trackIds = {songListDetail?.tracks?.length}
                        playCount = {songListDetail?.playCount} />
     <div className='songList'>
      <WYSongListTable columns={columns} tracks={getSongData(songListDetail?.tracks)} />
     </div>
     <WYErrorCover />
