...
import { getCurrentAlbumDetailAction,getHotAlbumsAction } from '../store/actionCreators'
...
export default memo(function WYAlbumDetail(props) {
  ...
  // 网络请求发送
  const {albumDetail,hotAlbumList,err} = useSelector(state => ({
  ...
  return (
    <AlbumDetailWrapper err={err} className='wrap-v2' isShowAlbumText={isShowAlbumText}>
      <div className='album-left'>
        <div className='album-title'>
          <div className='image'>
            <img src={albumDetail?.album?.picUrl} alt='' />
            <div className='cover sprite_covor'></div>
          </div>
          <div className='title'>
            <div className='title-name'>
              <span className='album-icon sprite_icon2'></span>
              <span className='text texts-nowrap'>{albumDetail?.album?.name}</span>
            </div>
            <div className='info'>
              <div className='artists'>歌手：<WYSongNameSort artists={albumDetail?.album?.artists} /> </div>
              <div>发行时间：{formatYearMonthDay(albumDetail?.album?.publishTime)}</div>
              {albumDetail?.album?.company ? <div>发行公司：{albumDetail?.album?.company}</div> : ''}
              
            </div>
            <WYPlayBtn share = {`(${albumDetail?.album?.info?.shareCount})`}
        ...
        <div className='album-text'>
          <div className='albumtext'>专辑介绍：</div> 
            {
             albumtext?.length > 3 ? albumtext?.slice(0,3).map((item,index) => {
                return (
            ...
            {
              isShowAlbumText ? albumtext?.slice(3).map((item,index) => {
                return (
                  <div key={index}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{item}</div>
                )
              }) : ''
        ...
        <WYSongListTitleCover title={'包含歌曲列表'}
                              trackIds={albumDetail?.songs?.length}
                              showCount={false} />
        <WYSongListTable columns={columns} tracks = {getSongdetailCover(albumDetail?.songs)}/>
      </div>
        ...
        <div className='user-img'>
          {
            images.map((item,index) => {
              return (
                <NavLink key={index} to={`/discover/artistDetail/${110}`}>
