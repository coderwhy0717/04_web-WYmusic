...
import { getPlaySongDetailAction,getMaskCoverAction } from '../../../store/actionCreators'
export default memo(function WYPlayDetail() {
    const [iscurrentplay,setIscurrentplay] = useState(0)
    const [isImage,setImage] = useState('')
    // maskCover 显示播放列表隐藏
    const {maskCover,playList,currentSong,playLyrics,lyricsIndex} = useSelector(state => ({
       ...
       <div className='maskCoverConent'>
           <div className='covertop playlist_bg'>
               ...
               <div className='title-right'>
                    ...
                    <div className='close playlist-c' onClick={e => changeClose()}></div>
                </div>
            </div>
            <div className='center playlist_bg'>
                ...
                <div className='songConent'  onScrollCapture={changeScroll}>
                  {
                      playList ? playList.map((item,index)=> {
                          return (
                            <div className={`song-detail ${index === iscurrentplay ? 'actives' : ''}`}
                                 key={item.id}
                                 onClick={e => changePlayMusic(item.id)} >
                                <div className={`play-icon ${index === iscurrentplay ? 'playlist-c' : ''}`}></div>
                                <div className='song-conent'>
                                    <div className='name'>{item.name}</div>
                                    <div className='user-name text-nowrap'>
                                        <WYUserNameSort  artists={item.ar} />
                                    </div>
                                    <div className='time'>{formatDate(item.dt,'mm:ss')}</div>
                                </div>
                ...
                <div className='lyConent'>
                    {
                        playLyrics.map((item,index) => {
                            return (
                                <div key={index} className={`lyitem ${lyricsIndex === index ? 'lyactive' : ''}`}>
                                    {item.content}
                                </div>
                            )
                        })
