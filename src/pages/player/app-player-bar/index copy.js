...
import { AppPlayerBarWrapper, PlayerBarCenter, PlayerBarLeft, PlayerBarRight } from './style'
import { shallowEqual, useDispatch, useSelector } from 'react-redux'
import { changePlayNextSongAndSequenceAction,
         changeSequenceAction, 
         getPlaySongDetailAction } from '../store/actionCreators'
...
export default memo(function WYAppPlayerBar() {
  ...
  // 组件的回调函数 进度条滑动/进度条点击
  const upChange = useCallback(value => {
      setIsChanging(false)
    ...
    <AppPlayerBarWrapper className='sprite_player'>
      <div className='wrap wrap-v2'>
        <PlayerBarLeft isPlaying={isPlaying}>
          ...
          <div className='play sprite_player' title="播放/暂停(p)" onClick={e => changePlayer()}> </div>
          <div className='rwa next sprite_player' title="下一首(ctrl+>)" onClick={e => playnext(1)}> </div>
        </PlayerBarLeft>
        <PlayerBarCenter>
          <div  className='image'>
          <img  src={getSizeImage(picUrl,35)} alt=''></img>
          <a href='/#' className='image-a sprite_player'> </a>
          </div>
          <div className='content'>
            <div className='content-top'>
              <a href='/#' className='title text-nowrap' title={currentSong.name}>
                {currentSong.name}
              </a>
        ...
        <PlayerBarRight sequence={sequence}>
          ...
          <div className='right sprite_player'>
            ...
            <div className="cycle btn sprite_player" title="循环" onClick={e => changeSequence()}></div>
            <div className="playlist btn sprite_player" title="播放列表">{playList.length}</div>
          </div>
        </PlayerBarRight>
      </div>
      <audio ref={audioRef}
